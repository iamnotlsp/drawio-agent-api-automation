...
| 已知缺陷（xfail） | 11 |
| 非预期失败 | 0 |
| Allure 请求/响应及日志附件 | 340 |

> 11 个 xfail 来自 8 个已确认缺陷，其中“无效智能体 ID”使用了 2 组参数化数据，“聊天必填参数校验”使用了 3 组参数化数据。
...
| BUG-010 | 创建会话和聊天接口无身份认证，调用方可伪造 userId 消耗其他用户额度 | `xfail(strict=True)` |
